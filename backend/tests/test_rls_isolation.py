...
async def test_rls_cross_workspace_isolation(two_workspaces: Any) -> None:
    ...
    try:
        ...
        pytest.skip(f"Database session unavailable: {e}")
        ...
        async with tenant_transaction_context(session, u2, ws2) as ctx:
            await ctx.execute(
                text(
                    ...
                    "VALUES (:id, :wid, 'Camp 2', :uid, 'draft')"
                ),
        ...
        with pytest.raises(ProgrammingError) as exc_info:
            async with tenant_transaction_context(session, u1, ws1) as ctx:
                await ctx.execute(
                    text(
                        "INSERT INTO campaigns (id, workspace_id, name, created_by, status) "
                        "VALUES (:id, :wid, 'Camp Invalid', :uid, 'draft')"
                    ),
                    {"id": str(camp_invalid_id), "wid": str(ws2), "uid": str(u1)},
                )
        assert (
            "insufficientprivilege" in str(exc_info.value).lower()
            or "row-level security" in str(exc_info.value).lower()
        ), "INSERT isolation failed: ws1 could insert a campaign into ws2"
