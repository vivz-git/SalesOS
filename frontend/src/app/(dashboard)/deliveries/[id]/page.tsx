    ...
    return (
      <div className="mx-auto max-w-4xl p-6">
        <div className="rounded-xl border border-rose-200 bg-rose-50 p-6 text-center text-rose-800">
          ...
          <div className="mt-4">
            <Link href="/inbox?tab=sent" className="text-xs font-semibold text-indigo-700 underline">
              Return to Inbox
            </Link>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-500">
              Recipient: <strong className="text-slate-900">{delivery.recipient_email}</strong>
            </p>
          </div>
        ...
        <div className="space-y-6 lg:col-span-2">
          <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-2xs space-y-4">
            ...
            <div className="space-y-3 text-xs">
              <div>
                <label className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Subject</label>
                <div className="mt-1 rounded-lg border border-slate-200 bg-slate-50 p-3 font-semibold text-slate-900">
                  {delivery.subject}
                </div>
              </div>

              <div>
                <label className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Body Content</label>
                <div className="mt-1 whitespace-pre-wrap rounded-lg border border-slate-200 bg-slate-50 p-4 text-slate-800 leading-relaxed min-h-[140px]">
                  {delivery.body}
                </div>
              </div>
            </div>
          ...
          {delivery.error_message && (
            <div className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-xs space-y-1">
              ...
              <p className="text-rose-800 text-[11px] bg-white p-2 rounded border border-rose-200">
                {delivery.error_message}
              </p>
            </div>
          )}
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-2xs space-y-3 text-xs">
            ...
            <div className="space-y-1.5 text-slate-600 text-[11px]">
              <div>Requested: <strong className="text-slate-800">{new Date(delivery.created_at).toLocaleString()}</strong></div>
              <div>Updated: <strong className="text-slate-800">{new Date(delivery.updated_at).toLocaleString()}</strong></div>
            </div>
          </div>
