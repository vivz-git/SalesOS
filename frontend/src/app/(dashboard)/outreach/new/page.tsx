...
import { createOutreachDraft } from "@/lib/api/outreach";
...
export default function NewOutreachDraftPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!activeWorkspace) return;
    if (!selectedCampaignId || !selectedContactId || !body.trim()) {
      setError("Campaign, Contact, and Message Body are required.");
      return;
    }
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 rounded-xl border border-zinc-200 bg-white p-6 shadow-2xs">
        ...
        {/* Subject Line Input */}
        <div className="space-y-2">
          <label htmlFor="subject_input" className="text-xs font-semibold text-zinc-900 uppercase tracking-wider">
            Subject Line
          </label>
          <input
            id="subject_input"
            type="text"
            placeholder="e.g. Quick question regarding SalesOS integration"
            value={subject}
        ...
        {/* Message Body Input */}
        <div className="space-y-2">
          <label htmlFor="body_input" className="text-xs font-semibold text-zinc-900 uppercase tracking-wider">
            Message Body <span className="text-rose-500">*</span>
          </label>
          <textarea
            id="body_input"
            rows={8}
            placeholder="Write personalized outreach message content here..."
            value={body}
            onChange={(e) => setBody(e.target.value)}
            className="w-full rounded-lg border border-zinc-200 bg-white p-3 text-sm font-mono text-zinc-900 focus:border-zinc-900 focus:outline-hidden"
            required
          />
        </div>
        ...
        <div className="flex items-center justify-end gap-3 border-t border-zinc-200 pt-4">
          ...
          >
            ...
            {submitting ? "Saving Draft..." : "Save Outreach Draft"}
          </button>
        </div>
      </form>
    </div>
