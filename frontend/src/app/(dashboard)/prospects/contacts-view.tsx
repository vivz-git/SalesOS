    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-zinc-200 pb-3">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
          <select
            ...
            className="rounded-md border border-zinc-300 px-3 py-1.5 text-xs text-zinc-900 focus:border-zinc-900 focus:outline-none"
          >
          ...
          <div className="relative w-full sm:w-56">
            ...
            <input
              ...
              className="w-full rounded-md border border-zinc-300 pl-8 pr-3 py-1.5 text-xs text-zinc-900 focus:border-zinc-900 focus:outline-none"
            />
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border bg-white p-6 text-center shadow-sm">
          ...
          <Button variant="outline" size="sm" onClick={loadContacts} className="mt-3">
            Retry
          </Button>
        </div>
      ) : contacts.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-zinc-300 bg-white p-12 text-center shadow-sm">
          <div className="flex h-12 w-12 items-center justify-center rounded-full bg-zinc-100 text-zinc-500">
            <Users className="h-6 w-6" />
          </div>
          <h3 className="mt-4 text-sm font-bold text-zinc-900">No contacts found</h3>
          ...
          {contacts.map((contact) => {
            ...
            return (
              <Link
                ...
                className="group flex flex-col justify-between rounded-xl border border-zinc-200 bg-white p-5 shadow-xs transition-all hover:border-zinc-300 hover:shadow-md"
              >
                <div>
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-1.5">
                      <h3 className="text-base font-bold text-zinc-900 group-hover:text-zinc-700">
                  ...
                  {contact.title && (
                    <p className="mt-1 text-xs font-semibold text-zinc-700">{contact.title}</p>
                  )}
      ...
      <div className="flex items-center justify-between border-t border-zinc-200 pt-4">
        <span className="text-xs text-zinc-500">
          Showing page <span className="font-semibold text-zinc-900">{page}</span>
        </span>
