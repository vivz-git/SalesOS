...
export default function ReportsPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-zinc-500">
            Real workspace activity aggregation, approval rate efficiency, delivery rates, and reply conversion metrics.
          </p>
        </div>
        ...
        <button
          ...
          disabled={generating}
          className="inline-flex items-center gap-1.5 rounded-lg bg-purple-600 px-4 py-2 text-xs font-semibold text-white hover:bg-purple-700 transition-colors shrink-0 shadow-2xs"
        >
      ...
      {loading ? (
        <div className="p-12 text-center text-xs text-zinc-400">Loading performance metrics and digest...</div>
      ) : selectedReport && m ? (
        <>
          ...
          <div className="flex items-center justify-between rounded-xl border border-zinc-200 bg-white p-4 shadow-2xs">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h2 className="text-sm font-bold text-zinc-900">{selectedReport.title}</h2>
            ...
            {reports.length > 1 && (
              <select
                ...
                className="rounded-lg border border-zinc-200 bg-zinc-50 p-2 text-xs font-semibold text-zinc-800"
              >
                {reports.map((rep) => (
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            <div className="rounded-xl border border-zinc-200 bg-white p-4 shadow-2xs space-y-1">
              <span className="text-xs font-semibold text-zinc-500 flex items-center gap-1">
                <TrendingUp className="h-3.5 w-3.5 text-purple-600" />
                Approval Rate
              </span>
              <div className="text-2xl font-bold text-purple-900">{m.approval_rate}%</div>
            ...
            <div className="rounded-xl border border-zinc-200 bg-white p-4 shadow-2xs space-y-1">
              ...
              <p className="text-[11px] text-zinc-400">HubSpot items updated</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <div className="space-y-6 lg:col-span-2">
              <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-2xs space-y-4">
                ...
                <p className="text-xs text-zinc-700 leading-relaxed whitespace-pre-wrap bg-purple-50/40 p-4 rounded-lg border border-purple-100 font-sans">
              ...
              <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-2xs space-y-4">
                ...
                <ul className="space-y-2 text-xs text-zinc-700">
                  {selectedReport.recommended_actions.map((act, idx) => (
                    <li key={idx} className="flex items-start gap-2 bg-zinc-50 p-3 rounded-lg border border-zinc-100">
            ...
            <div className="space-y-6">
              <div className="rounded-xl border border-zinc-200 bg-white p-5 shadow-2xs space-y-4 text-xs">
                ...
                <div className="space-y-2.5 divide-y divide-zinc-100">
                  <div className="flex justify-between pt-1">
                    ...
                    <span className="font-bold text-zinc-900">{m.campaigns_count}</span>
                  </div>
                  <div className="flex justify-between pt-2">
                    <span className="text-zinc-500">Accounts Researched</span>
                    <span className="font-bold text-zinc-900">{m.accounts_researched_count}</span>
                  </div>
                  <div className="flex justify-between pt-2">
                    <span className="text-zinc-500">Contacts Enrolled</span>
                    <span className="font-bold text-zinc-900">{m.contacts_enrolled_count}</span>
                  </div>
                  <div className="flex justify-between pt-2">
                    <span className="text-zinc-500">Drafts Generated</span>
                    <span className="font-bold text-zinc-900">{m.drafts_generated_count}</span>
                  </div>
                  <div className="flex justify-between pt-2">
                    <span className="text-zinc-500">Drafts Submitted</span>
                    <span className="font-bold text-zinc-900">{m.drafts_submitted_count}</span>
                  </div>
                  <div className="flex justify-between pt-2">
                    <span className="text-zinc-500">Drafts Approved</span>
                    <span className="font-bold text-zinc-900">{m.drafts_approved_count}</span>
                  </div>
                  <div className="flex justify-between pt-2">
                    <span className="text-zinc-500">Emails Bounced</span>
                    <span className="font-bold text-rose-700">{m.emails_bounced_count}</span>
                  </div>
                  <div className="flex justify-between pt-2">
                    <span className="text-zinc-500">Opt-Out Rate</span>
                    <span className="font-bold text-rose-700">{m.opt_out_rate}%</span>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-blue-200 bg-blue-50/70 p-4 text-xs text-blue-900 flex items-start gap-2">
                ...
                <span>
                  Deterministic metrics: Calculated from stored SalesOS records. Zero autonomous sending occurs.
                </span>
              </div>
            </div>
          </div>
