    ...
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-6 shadow-2xs space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-zinc-100 pb-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-zinc-500">
              Idempotent synchronization of Contacts, Companies, Outbound Email Deliveries, and Inbound Prospect Replies.
            </p>
          </div>
            ...
            {connection?.status === "connected" ? (
              <>
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-lg border border-zinc-200 px-3.5 py-2 text-xs font-semibold text-zinc-600 hover:bg-zinc-100 transition-colors"
                >
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-lg bg-amber-600 px-4 py-2 text-xs font-semibold text-white hover:bg-amber-700 transition-colors shadow-2xs"
              >
        ...
        {connection && connection.status === "connected" && (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 text-xs bg-zinc-50 p-4 rounded-xl border border-zinc-200">
            <div>
              <span className="font-semibold text-zinc-500 block">HubSpot Portal ID</span>
              <span className="font-mono text-zinc-900 font-bold">{connection.portal_id || "portal-998877"}</span>
            </div>
            <div>
              <span className="font-semibold text-zinc-500 block">Connected At</span>
              <span className="text-zinc-900">{connection.connected_at ? new Date(connection.connected_at).toLocaleString() : "Active"}</span>
            </div>
            <div>
              <span className="font-semibold text-zinc-500 block">Last Synced</span>
              <span className="text-zinc-900">{connection.last_synced_at ? new Date(connection.last_synced_at).toLocaleString() : "Never"}</span>
            </div>
          </div>
        )}
        ...
        {connection?.status === "connected" && (
          <div className="space-y-3 pt-2">
            ...
            <div className="rounded-xl border border-zinc-200 overflow-hidden">
              {loading ? (
                <div className="p-8 text-center text-xs text-zinc-400">Loading sync history...</div>
              ) : syncRuns.length === 0 ? (
                <div className="p-8 text-center text-xs text-zinc-400">No sync runs recorded yet.</div>
              ) : (
                  ...
                  <tbody className="divide-y divide-zinc-100">
                    {syncRuns.map((run) => (
                      <tr key={run.id} className="hover:bg-zinc-50">
                        <td className="px-4 py-2.5 font-mono text-[11px] text-zinc-600">{run.id.substring(0, 8)}...</td>
                        ...
                        <td className="px-4 py-2.5 font-semibold text-zinc-900">{run.records_processed} items</td>
        ...
        <div className="rounded-xl border border-purple-200 bg-purple-50/60 p-4 text-xs text-purple-900 flex items-start gap-2">
          ...
          <span>
            Server-side token protection: OAuth tokens are stored encrypted and processed server-side. Zero auto-sending occurs.
          </span>
        </div>
      </div>
    </div>
