...
export function Sidebar({
      ...
      <div>
        <div className="flex items-center justify-between px-4 pb-4">
          ...
          <button
            ...
            className="rounded-md p-1 text-zinc-500 hover:bg-zinc-100 hover:text-zinc-900 md:hidden"
        ...
        <nav aria-label="Sidebar" className="mt-2 grid gap-1 px-2">
          {NAVIGATION_ITEMS.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={`h-5 w-5 shrink-0 ${active ? "text-white" : "text-zinc-500 group-hover:text-zinc-900"}`} />
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside
        className={`hidden border-r border-zinc-200 bg-white transition-all duration-300 md:block ${
          collapsed ? "w-16" : "w-64"
        }`}
      >
        {content}
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 md:hidden">
          <div
            ...
            onClick={onCloseMobile}
          />
          <aside className="relative z-50 h-full w-64 border-r border-zinc-200 bg-white shadow-xl">
            {content}
          </aside>
        </div>
      )}
