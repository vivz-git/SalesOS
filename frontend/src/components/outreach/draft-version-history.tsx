...
export function DraftVersionHistory({
 ...
 return (
 <div key={ver.id} className="p-3.5 transition-colors hover:bg-slate-50/50">
 ...
 {isAi ? <Bot className="h-3 w-3 text-slate-600"/> : <User className="h-3 w-3 text-slate-400"/>}
 ...
 className="rounded px-2 py-1 text-xs font-medium text-slate-600 hover:bg-slate-200 hover:text-slate-900"
 >
 View
 </button>
 )}
 ...
 {isExpanded && (
 <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-slate-800 whitespace-pre-wrap font-mono">
 {ver.body}
 </div>
 )}
 </div>
