...
export function EvidenceViewer({ brief }: EvidenceViewerProps) {
 ...
 <span className="text-xs text-slate-400 font-medium">Score Pending</span>
 )}
 </div>

 {brief.confidence_reason ? (
 <div className="rounded-lg border border-emerald-100 bg-white p-3 text-xs text-slate-700">
 <span className="font-semibold text-slate-900 block mb-0.5">Evaluation Reason:</span>
 {brief.confidence_reason}
 </div>
 ) : (
 ...
 <div className="grid grid-cols-2 gap-2 text-[11px] text-slate-600 sm:grid-cols-4">
 {brief.provider && (
 <div>
 <span className="font-medium text-slate-400 block">Provider</span>
 <span className="font-semibold text-slate-900">{brief.provider}</span>
 </div>
 )}
 {brief.model && (
 <div>
 <span className="font-medium text-slate-400 block">Model</span>
 <span className="font-semibold text-slate-900">{brief.model}</span>
 </div>
 )}
 {brief.prompt_version && (
 <div>
 <span className="font-medium text-slate-400 block">Prompt Version</span>
 <span className="font-semibold text-slate-900">{brief.prompt_version}</span>
 </div>
 )}
 {brief.token_usage !== null && (
 <div>
 <span className="font-medium text-slate-400 block">Token Usage</span>
 <span className="font-semibold text-slate-900">{brief.token_usage} tokens</span>
 </div>
 )}
 </div>
 </div>
