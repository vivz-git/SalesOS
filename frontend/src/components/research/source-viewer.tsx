...
export function SourceViewer({ sources }: SourceViewerProps) {
 ...
 return (
 <div className="rounded-xl border border-dashed border-slate-300 bg-white p-6 text-center shadow-xs">
 <p className="text-xs font-medium text-slate-500">No source provenance records attached yet.</p>
 </div>
 );
 }
 ...
 <div className="divide-y rounded-xl border border-slate-200 bg-white shadow-xs">
 {sources.map((source) => (
 ...
 {source.snippet && (
 <p className="text-slate-600 text-xs italic bg-slate-50 rounded p-2 border border-slate-100">
 &quot;{source.snippet}&quot;
 </p>
 )}

 <div className="flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 pt-2 text-[11px] text-slate-400">
