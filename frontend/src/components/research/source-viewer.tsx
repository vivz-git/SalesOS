...
export function SourceViewer({ sources }: SourceViewerProps) {
  if (!sources || sources.length === 0) {
    return (
      <div className="rounded-xl border border-dashed border-zinc-300 bg-white p-6 text-center shadow-xs">
        <p className="text-xs font-medium text-zinc-500">No source provenance records attached yet.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="divide-y rounded-xl border border-zinc-200 bg-white shadow-xs">
        {sources.map((source) => (
          <div key={source.id} className="p-4 space-y-2 text-xs">
            <div className="flex items-start justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <span className="rounded bg-zinc-100 px-1.5 py-0.5 text-[10px] font-semibold text-zinc-600 uppercase">
                  {source.source_type}
                </span>
              </div>
            ...
            {source.snippet && (
              <p className="text-zinc-600 text-xs italic bg-zinc-50 rounded p-2 border border-zinc-100">
                &quot;{source.snippet}&quot;
              </p>
            )}

            <div className="flex flex-wrap items-center justify-between gap-2 border-t border-zinc-100 pt-2 text-[11px] text-zinc-400">
