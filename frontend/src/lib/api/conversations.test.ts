...
describe("Conversations API Client", () => {
  ...
  it("fetchConversations requests GET /v1/conversations with parameters", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      "/v1/conversations?status=active",
      expect.objectContaining({ headers: expect.any(Headers) })
    );
    expect(results).toHaveLength(1);
  });

  it("fetchConversationDetail requests GET /v1/conversations/:id", async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
    ...
    expect(mockFetch).toHaveBeenCalledWith("/v1/conversations/c1", expect.any(Object));
    expect(detail.status).toBe("needs_human_action");
  });

  it("ingestInboundReply sends POST to /v1/conversations/inbound", async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
  ...
  it("reclassifyConversation sends POST to classify action endpoint", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      "/v1/conversations/c1/actions/classify",
      expect.objectContaining({ method: "POST" })
    );
  ...
  it("updateConversationStatus sends POST to status action endpoint", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      "/v1/conversations/c1/actions/update-status",
      expect.objectContaining({ method: "POST" })
    );
