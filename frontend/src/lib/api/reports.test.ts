...
describe("Reports API Client", () => {
 ...
 it("fetchWeeklyReportsList requests GET /api/v1/reports/weekly", async () => {
...
"/api/v1/reports/weekly?limit=10&offset=0",
 expect.objectContaining({ headers: expect.any(Headers) })
 );
 expect(reports).toHaveLength(1);
 });

 it("fetchWeeklyReportDetail requests GET /api/v1/reports/weekly/:id", async () => {
 mockFetch.mockResolvedValueOnce({
 ok: true,
 ...
 expect(mockFetch).toHaveBeenCalledWith("/api/v1/reports/weekly/rep-1", expect.any(Object));
 expect(detail.id).toBe("rep-1");
 });

 it("generateWeeklyReport sends POST to /api/v1/reports/weekly/actions/generate", async () => {
 mockFetch.mockResolvedValueOnce({
 ok: true,
 ...
 expect(mockFetch).toHaveBeenCalledWith(
"/api/v1/reports/weekly/actions/generate",
